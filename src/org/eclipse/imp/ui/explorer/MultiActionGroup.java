...
/**
 * A MultiActionGroup will display a list of IActions in a menu by transforming them into MenuItems. The list of labels given will be what is displayed in the
 * ViewMenu for the corresponding action (the action at the same position in the action array). The actions are currently implemented as state based so that
 * after an action is executed the label will have a selection check.
 * 
 * @since 2.1
 */
public class MultiActionGroup extends ActionGroup {
     ...
     *            decides which action is selected in the menu on start up. Denotes the location in the actions array of the current selected state. It cannot
     *            be null.
     */
    ...
    /**
     * Creates a new action group. Clients using this constructor must set the actions immediately after creating the multi action group by calling
     * {@link #setActions(IAction[], int)}.
     */
     ...
     * 
     * @param actions
     *            the actions for this multi group, at least one
     * @param currentSelection
     *            decides which action is selected in the menu on start up. Denotes the location in the actions array of the current selected state. It cannot
     *            be null.
     */
        ...
        for(int i= 0; i < fActions.length; i++) {
            ...
            viewMenu.add(new ContributionItem() {
                public void fill(Menu menu, int index) {
                    ...
                    mi.setImage(RuntimePlugin.getImageDescriptorRegistry().get(d));
                    fItems[j]= mi;
