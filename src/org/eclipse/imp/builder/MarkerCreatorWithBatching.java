...
//	@Override public void handleProblem(ProblemHandler.Severity problemSeverity, ProblemHandler.Phase processingPhase,
	public void handleProblem(int problemSeverity, String contextPrefix,
			String problemMessage, String processingContext, int startOffset, int endOffset)
	throws ProblemLimit.LimitExceededException
	{
		int line = -1;
		if (startOffset >= 0) {
			// abs to move non-token to predecessor
			int leftToken = Math.abs(getParseStream().getTokenIndexAtCharacter(startOffset));
			if (leftToken >= 0)
				line = getParseStream().getLine(leftToken);
		}
		Integer severity = severityMap.get(problemSeverity);
		addMarker(severity != null ? severity.intValue() : IMarker.SEVERITY_ERROR,
				contextPrefix + ": " + problemMessage, line, startOffset, endOffset+1);
	}
